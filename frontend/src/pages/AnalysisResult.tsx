...
export default function AnalysisResult() {
  ...
  if (error || !analysis) return <div className="mx-auto max-w-5xl rounded border border-red-800/60 bg-red-950/30 p-5 text-sm text-critical">{error || 'Analysis not found.'}</div>;
  ...
  return <div className="mx-auto max-w-[1500px] space-y-6 pb-14 text-gray-200">
    ...
    <nav className="flex flex-wrap gap-2 border-b border-hairline pb-4 font-mono text-[10px] uppercase tracking-widest text-gray-500">{['Verdict', 'Why flagged', 'Mail flow', 'Authentication', 'Headers', 'Content', 'Email preview', 'Infrastructure', 'Raw email'].map((item, index) => <a key={item} href={`#${item === 'Why flagged' ? 'findings' : slug(item)}`} className="rounded border border-hairline-strong px-2.5 py-1.5 hover:border-cyan-700 hover:text-accent"><span className="mr-1.5 text-accent/70">{String(index + 1).padStart(2, '0')}</span>{item}</a>)}</nav>
...
function DetectionEvidencePanel({ analysis }: { analysis: RecordValue }) {
  ...
  return <section className="rounded-lg border border-cyan-500/25 bg-surface p-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><p className="report-kicker">Judge-visible evidence</p><h2 className="mt-1 text-xl font-semibold text-white">Why this message is suspicious</h2></div><span className="font-mono text-[10px] uppercase tracking-widest text-gray-500">{detectorItems.length} detector signals</span></div>{detectorItems.length ? <div className="mt-4 grid gap-3 md:grid-cols-2">{detectorItems.slice(0, 6).map((item: RecordValue, index: number) => <div key={`${item.anomaly_type || item.type || 'signal'}-${index}`} className="rounded border border-hairline-strong bg-raised p-4"><div className="flex items-start justify-between gap-3"><div className="flex items-start gap-2"><CircleAlert size={15} className="mt-0.5 shrink-0 text-accent" /><p className="text-sm font-medium text-gray-200">{item.description || 'Detection signal observed'}</p></div><span className={`shrink-0 font-mono text-[10px] uppercase ${item.severity === 'critical' || item.severity === 'high' ? 'text-critical' : 'text-medium'}`}>{item.severity || 'signal'}</span></div><p className="mt-2 font-mono text-[10px] text-gray-500">Confidence {item.confidence || 0}% · {item.evidence || item.observed_domain || 'Evidence recorded'}</p></div>)}</div> : <p className="mt-4 rounded border border-dashed border-hairline-strong p-4 text-sm text-gray-500">No anomaly or impersonation signal was returned for this message.</p>}</section>;
}
...
function TacticalAlertBanner({ analysis, verdict, risk, subject, certDraft, evidenceHash, ledger, id }: { analysis: RecordValue; verdict: string; risk: number; subject: string; certDraft: string; evidenceHash?: string; ledger: RecordValue; id?: string }) {
  ...
  return <section className={`overflow-hidden rounded-lg border ${risk >= 70 ? 'border-red-500/60 bg-red-950/25' : 'border-amber-500/50 bg-amber-950/20'} shadow-[0_12px_50px_rgba(0,0,0,0.2)]`}>
    <div className="flex flex-col gap-4 border-b border-white/10 p-5 lg:flex-row lg:items-center lg:justify-between">
      <div className="flex items-start gap-3"><div className={`rounded border p-2 ${risk >= 70 ? 'border-red-400/50 text-critical' : 'border-amber-400/50 text-medium'}`}><ShieldAlert size={22} /></div><div><p className="font-mono text-[10px] uppercase tracking-[0.2em] text-gray-400">Tactical incident alert</p><h2 className="mt-1 text-xl font-bold uppercase text-white">{classification} <span className="text-gray-400">· {analysis.attack_type || 'CREDENTIAL HARVESTING'}</span></h2></div></div>
      <div className="flex shrink-0 items-center gap-2 font-mono text-[10px] text-gray-400"><Radio size={14} className="animate-pulse text-critical" /> Incident dossier {id ? id.slice(0, 8) : 'local'}</div>
    </div>
    ...
    <section className="rounded-lg border border-hairline-strong bg-surface p-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2 text-[10px] font-mono uppercase tracking-widest">
          {cleanCount > 0 && <span className="rounded border border-emerald-500/50 bg-emerald-950/30 px-2 py-1 text-safe">{cleanCount}/6 layers → CLEAN</span>}
          {suspiciousCount > 0 && <span className="rounded border border-amber-500/50 bg-amber-950/30 px-2 py-1 text-medium">{suspiciousCount}/6 layers → SUSPICIOUS</span>}
          {maliciousCount > 0 && <span className="rounded border border-red-500/50 bg-red-950/30 px-2 py-1 text-critical">{maliciousCount}/6 layers → MALICIOUS</span>}
        </div>
      </div>
...
function ThreatHopTimeline({ hops, origin }: { hops: RecordValue[]; origin?: string }) {
  ...
  return <div className="mb-5 rounded border border-cyan-500/20 bg-sunken p-4"><div className="flex items-center gap-2 text-[10px] uppercase tracking-widest text-accent"><MapPin size={14} /> Interactive threat hop timeline</div><div className="mt-4 flex gap-2 overflow-x-auto pb-2">{hops.map((hop, index) => <button type="button" key={`${hop.source_ip || hop.from_server || index}`} onClick={() => setSelected(index)} className={`flex min-w-[150px] items-center gap-2 rounded border px-3 py-2 text-left ${selected === index ? 'border-red-400/70 bg-red-950/20' : 'border-hairline-strong bg-surface'}`}><span className={`h-2 w-2 rounded-full ${selected === index ? 'bg-red-400 animate-pulse' : 'bg-cyan-400'}`} /><span><span className="block text-[10px] text-gray-500">Hop {index + 1}</span><span className="block truncate font-mono text-xs text-gray-200">{hop.by_server || hop.by_host || hop.from_server || 'Relay'}</span></span>{index < hops.length - 1 && <span className="text-accent/70">→</span>}</button>)}</div><div className="mt-2 grid gap-2 text-xs text-gray-400 sm:grid-cols-4"><span>Origin: <strong className="font-mono text-gray-200">{origin || active?.source_ip || 'unavailable'}</strong></span><span>WHOIS: <strong className="text-gray-200">{active?.whois?.organization || active?.whois?.registrar || 'unavailable'}</strong></span><span>ISP: <strong className="text-gray-200">{active?.isp || active?.geolocation?.isp || 'unavailable'}</strong></span><span>Geo: <strong className="text-gray-200">{active?.geolocation ? [active.geolocation.city, active.geolocation.country].filter(Boolean).join(', ') : 'unavailable'}</strong></span></div></div>;
}
...
function IntegrityBox({ evidenceHash, ledger, analysisId }: { evidenceHash?: string; ledger: RecordValue; analysisId?: string }) {
  ...
  const statusClass = status === 'match' ? 'border-emerald-400/40 text-safe' : status === 'mismatch' ? 'border-red-400/40 text-critical' : status ? 'border-amber-400/40 text-medium' : 'border-gray-600 text-gray-500';
  const etherscanUrl = verification?.etherscan_url || (txHash ? `https://sepolia.etherscan.io/tx/${txHash}` : undefined);
  return <section className="rounded-lg border border-emerald-500/30 bg-emerald-950/10 p-5"><div className="flex flex-wrap items-center justify-between gap-3"><div className="flex items-center gap-2"><ShieldCheck size={20} className="text-safe" /><div><p className="report-kicker">Court-admissible evidence integrity</p><h2 className="mt-1 text-xl font-semibold text-white">Cryptographic chain of custody</h2></div></div><span className={`inline-flex items-center gap-2 rounded border px-2 py-1 font-mono text-[10px] uppercase ${statusClass}`}>{status === 'match' ? <Check size={13} /> : <Fingerprint size={13} />}{statusLabel}</span></div><div className="mt-5 grid gap-3 md:grid-cols-3"><div className="rounded border border-hairline-strong bg-sunken p-3"><p className="report-kicker">Raw email SHA-256 digest</p><p className="mt-2 break-all font-mono text-xs text-gray-300">{verification?.local_hash || evidenceHash || 'Not returned by analysis pipeline'}</p></div><div className="rounded border border-hairline-strong bg-sunken p-3"><p className="report-kicker">Blockchain anchor</p><p className="mt-2 font-mono text-sm text-gray-200">{verification?.network || ledger.network || ledger.chain || 'Sepolia Testnet (Ethereum)'}</p>{verification?.on_chain_hash && <p className="mt-2 break-all font-mono text-[10px] text-gray-500">On-chain: {verification.on_chain_hash}</p>}</div><div className="rounded border border-hairline-strong bg-sunken p-3"><p className="report-kicker">Smart contract transaction</p>{etherscanUrl ? <a href={etherscanUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-2 break-all font-mono text-xs text-accent hover:text-accent">{verification?.tx_hash || txHash}<ExternalLink size={13} /></a> : <p className="mt-2 font-mono text-xs text-gray-500">No ledger transaction recorded</p>}</div></div><div className="mt-4 flex flex-wrap items-center gap-3"><button type="button" onClick={verify} disabled={verifying || !analysisId} className="btn-secondary disabled:cursor-wait disabled:opacity-60">{verifying ? 'VERIFYING...' : 'Verify →'}</button>{error && <span className="text-xs text-critical">{error}</span>}{verification?.block_number && <span className="font-mono text-[10px] text-gray-500">Block {verification.block_number}</span>}</div></section>;
}
...
function ReportSection({ id, title, subtitle, children }: { id: string; title: string; subtitle: string; children: ReactNode }) {
  ...
  if (secondary) {
    return <details id={id} className="scroll-mt-5 border-t border-hairline-strong pt-5 group"><summary className="cursor-pointer list-none"><p className="report-kicker">{subtitle}</p><div className="mt-1 flex items-center justify-between gap-4"><h2 className="text-xl font-semibold text-white">{title}</h2><span className="font-mono text-[10px] uppercase tracking-widest text-accent group-open:text-gray-500">{id === 'raw-email' ? 'Open source' : 'Expand evidence'}</span></div></summary><div className="mt-4">{children}</div></details>;
  }
  return <section id={id} className="scroll-mt-5 border-t border-hairline-strong pt-5"><p className="report-kicker">{subtitle}</p><h2 className="mt-1 text-xl font-semibold text-white">{title}</h2><div className="mt-4">{children}</div></section>;
}
...
function EvidenceTimeline({ stages }: { stages: RecordValue[] }) {
  ...
  return <section id="evidence-timeline" className="scroll-mt-5 border-t border-hairline-strong pt-5"><div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><p className="report-kicker">Pipeline trace</p><h2 className="mt-1 text-xl font-semibold text-white">Analysis Evidence Timeline</h2><p className="mt-2 text-xs text-gray-500">Measured processing stages from ingestion to persisted verdict. This is execution evidence, not generated narrative.</p></div><div className="font-mono text-xs text-gray-500">{stages.length ? `${formatDuration(totalDuration)} total trace` : 'Timing data unavailable'}</div></div>{stages.length ? <div className="mt-5 overflow-hidden rounded border border-hairline-strong bg-surface"><div className="grid grid-cols-[2rem_1fr_auto] gap-3 border-b border-hairline-strong bg-sunken px-4 py-3 report-kicker"><span>#</span><span>Stage</span><span>Duration</span></div><div className="divide-y divide-[#1b3037]">{stages.map((stage, index) => { const duration = Number(stage.duration_ms || stage.duration || 0); const status = String(stage.status || 'completed').toLowerCase(); const failed = ['error', 'failed', 'timeout'].includes(status); return <div key={`${stage.stage || 'stage'}-${index}`} className="grid grid-cols-[2rem_1fr_auto] gap-3 px-4 py-3 text-xs"><div className="flex items-start justify-center pt-0.5 text-gray-600">{failed ? <XCircle size={14} className="text-critical" /> : status === 'running' ? <CircleDot size={14} className="text-medium" /> : <CheckCircle2 size={14} className="text-safe" />}</div><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-mono text-gray-200">{stage.stage || 'Unnamed stage'}</span><span className={`font-mono text-[10px] uppercase ${failed ? 'text-critical' : status === 'running' ? 'text-medium' : 'text-safe'}`}>{status}</span>{stage.item_count !== undefined && <span className="font-mono text-[10px] text-gray-600">{stage.item_count} items</span>}</div><div className="mt-2 h-1 overflow-hidden rounded-full bg-[#1b3037]"><div className={`h-full ${failed ? 'bg-red-400' : 'bg-cyan-400'}`} style={{ width: `${Math.max(duration ? 3 : 0, Math.min(100, duration / maxDuration * 100))}%` }} /></div>{stage.error && <p className="mt-2 text-[11px] text-critical">{stage.error}</p>}</div><span className="whitespace-nowrap font-mono text-gray-400">{formatDuration(duration)}</span></div>; })}</div></div> : <div className="mt-5 rounded border border-dashed border-hairline-strong px-4 py-8 text-center text-xs text-gray-600">Older analyses do not contain stage timing records.</div>}</section>;
}
*/
...
function ProviderTable({ results }: { results: any }) { const providers = [...new Map((Array.isArray(results) ? results : []).map((item: RecordValue) => [item.provider, item.status || 'unknown'])).entries()]; return <div className="overflow-hidden rounded border border-hairline-strong"><div className="grid grid-cols-[1fr_auto] bg-surface px-3 py-2 report-kicker"><span>Provider</span><span>Status</span></div>{providers.length ? providers.map(([name, status]) => <div key={name} className="grid grid-cols-[1fr_auto] border-t border-hairline-strong px-3 py-3 text-xs"><span className="text-gray-300">{name}</span><span className={`font-mono uppercase ${status === 'not_found' ? 'text-safe' : status === 'timeout' || status === 'error' ? 'text-medium' : 'text-gray-400'}`}>{status}</span></div>) : <p className="p-4 text-xs text-gray-600">No provider results returned.</p>}</div>; }
function IpEnrichmentTable({ results }: { results: any }) { const items = Array.isArray(results) ? results : []; return <div className="mt-4 overflow-hidden rounded border border-hairline-strong"><div className="bg-surface px-3 py-2 report-kicker">Reverse lookup and geolocation</div>{items.length ? items.map((item: RecordValue) => <div key={item.ip} className="border-t border-hairline-strong px-3 py-3 text-xs"><div className="flex flex-wrap justify-between gap-2"><span className="font-mono text-gray-200">{item.ip}</span><span className="uppercase text-gray-500">{item.classification}</span></div><div className="mt-2 grid gap-1 text-gray-400 sm:grid-cols-2"><span>PTR: <strong className="text-gray-200">{item.reverse_dns?.hostname || item.reverse_dns?.status || 'unavailable'}</strong></span><span>Location: <strong className="text-gray-200">{item.geolocation ? [item.geolocation.city, item.geolocation.country].filter(Boolean).join(', ') : 'unavailable'}</strong></span></div><p className="mt-1 font-mono text-[10px] text-gray-600">{item.provenance?.reverse_dns || 'reverse_dns_ptr'} · {item.provenance?.geolocation || 'no geolocation provider'}</p></div>) : <p className="p-4 text-xs text-gray-600">No public IPs available for reverse lookup.</p>}</div>; }
...
function IocMetric({ icon, label, value }: { icon: React.ReactNode; label: string; value: number }) { return <div className="rounded border border-hairline-strong bg-surface p-3"><div className="flex items-center gap-2 text-xs text-gray-500">{icon}{label}</div><p className="mt-2 font-mono text-xl text-gray-200">{value}</p></div>; }
...
function Empty({ text }: { text: string }) { return <p className="rounded border border-dashed border-hairline-strong p-5 text-center text-xs text-gray-600">{text}</p>; }
...
function verdictTone(value: string) { return value === 'malicious' ? 'text-critical' : value === 'suspicious' ? 'text-medium' : value === 'benign' ? 'text-safe' : 'text-gray-300'; }
...
function authTone(value?: string) { return value === 'pass' ? 'text-safe' : ['fail', 'reject'].includes(value || '') ? 'text-critical' : 'text-gray-500'; }
